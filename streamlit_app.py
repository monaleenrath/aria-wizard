...
_DOMAIN_SIGNALS: dict[str, list[str]] = {
    ...
    "streaming":    ["subscriber","subscription","cancellation","watch_hour","watch_time",
                     "content_cost","streaming","plan_type","churn","titles","episodes",
                     "new_subscription","active_subscriber","avg_watch","content_type"],
    "grocery":      ["basket","loyalty","redemption","waste","promo_revenue","cogs",
                     "loyalty_member","new_loyalty","gross_profit","avg_basket",
                     "province","category"],
}
...
_DIRECT_PATTERNS: list[dict] = [
    ...
    # ── Streaming & Media ─────────────────────────────────────────────────── #
    {"pattern": r"(active.?subscriber|total.?subscriber|subscriber.?count|^subscribers$)",
     "name": "Active Subscribers","agg": "sum",   "format": "integer",
     "description": "Total active subscribers at end of period."},
    {"pattern": r"(new.?subscription|new.?subscriber|gross.?add|subscriber.?add)",
     "name": "New Subscriptions","agg": "sum",    "format": "integer",
     "description": "New subscriptions / gross subscriber additions in the period."},
    {"pattern": r"(cancellation|churn.?count|^churned$|subscriber.?loss|lost.?sub)",
     "name": "Cancellations",   "agg": "sum",     "format": "integer",
     "description": "Total subscription cancellations / churned subscribers."},
    {"pattern": r"(watch.?hour|view.?hour|avg.?watch|hours.?per.?user|watch.?time)",
     "name": "Avg Watch Hours", "agg": "mean",    "format": "decimal",
     "description": "Average watch / view hours per subscriber in the period."},
    {"pattern": r"(content.?cost|content.?spend|programming.?cost|content.?expense)",
     "name": "Content Cost",    "agg": "sum",     "format": "currency",
     "description": "Total content acquisition / production spend."},
    {"pattern": r"(ops.?cost|operational.?cost|tech.?cost|platform.?cost|infra.?cost)",
     "name": "Ops Cost",        "agg": "sum",     "format": "currency",
     "description": "Technology / infrastructure / operational cost."},
    {"pattern": r"(^titles?$|titles?.?available|content.?count|catalogue.?size)",
     "name": "Titles Available","agg": "mean",    "format": "integer",
     "description": "Total titles / content pieces available on the platform."},
    # ── Grocery & FMCG ───────────────────────────────────────────────────── #
    {"pattern": r"(transaction.?count|^transactions$|basket.?count|visit.?count)",
     "name": "Transactions",    "agg": "sum",     "format": "integer",
     "description": "Total customer transactions / store visits."},
    {"pattern": r"(avg.?basket|basket.?size|spend.?per.?visit|avg.?spend.?per)",
     "name": "Avg Basket",      "agg": "mean",    "format": "currency",
     "description": "Average basket / transaction value per visit."},
    {"pattern": r"(loyalty.?redemption|redemption.?count|points.?redeemed)",
     "name": "Loyalty Redemptions","agg": "sum",  "format": "integer",
     "description": "Total loyalty points / vouchers redeemed."},
    {"pattern": r"(loyalty.?member|member.?count|^loyalty_members$)",
     "name": "Loyalty Members", "agg": "mean",    "format": "integer",
     "description": "Total active loyalty programme members."},
    {"pattern": r"(new.?loyalty|new.?member|loyalty.?join|member.?join)",
     "name": "New Loyalty Members","agg": "sum",  "format": "integer",
     "description": "New loyalty programme sign-ups in the period."},
    {"pattern": r"(^waste$|waste.?cad|waste.?usd|waste.?amount|shrinkage.?amount)",
     "name": "Waste ($)",       "agg": "sum",     "format": "currency",
     "description": "Total food / product waste value — key grocery cost driver."},
    {"pattern": r"(waste.?pct|shrinkage.?pct|spoilage.?rate|^waste_pct$)",
     "name": "Waste %",         "agg": "mean",    "format": "percent",
     "description": "Waste as % of revenue. Industry benchmark: < 5%."},
    {"pattern": r"(promo.?revenue|promotional.?sales|promo.?sales|deal.?revenue)",
     "name": "Promo Revenue",   "agg": "sum",     "format": "currency",
     "description": "Revenue generated through promotional / discounted pricing."},
    {"pattern": r"(gross.?profit|^gp$|gross.?margin.?amount|gross.?income)",
     "name": "Gross Profit",    "agg": "sum",     "format": "currency",
...
_DERIVED_PATTERNS: list[dict] = [
    ...
    # ── Streaming & Media ─────────────────────────────────────────────────── #
    {"num_pattern": r"(cancellation|churn.?count|^churned$|subscriber.?loss|lost.?sub)",
     "den_pattern": r"(active.?subscriber|total.?subscriber|subscriber.?count|^subscribers$)",
     "name": "Churn Rate %",    "scale": 1, "format": "percent",
     "formula_tmpl": "SUM({num}) ÷ SUM({den})",
     "description": "Subscriber cancellations as % of active base. Key retention health metric."},
    {"num_pattern": r"(content.?cost|content.?spend|programming.?cost|content.?expense)",
     "den_pattern": r"(sale|revenue|gmv|revenue_usd|revenue_cad)",
     "name": "Content Cost %",  "scale": 1, "format": "percent",
     "formula_tmpl": "SUM({num}) ÷ SUM({den})",
     "description": "Content spend as % of revenue. Streaming benchmark: < 60%."},
    # ── Grocery & FMCG ───────────────────────────────────────────────────── #
    {"num_pattern": r"(gross.?profit|^gp$|gross.?margin.?amount|gross.?income)",
     "den_pattern": r"(sale|revenue|gmv|revenue_usd|revenue_cad)",
     "name": "Gross Margin %",  "scale": 1, "format": "percent",
     "formula_tmpl": "SUM({num}) ÷ SUM({den})",
     "description": "Gross profit as % of revenue. Grocery benchmark: 25–35%."},
    {"num_pattern": r"(promo.?revenue|promotional.?sales|promo.?sales|deal.?revenue)",
     "den_pattern": r"(sale|revenue|gmv|revenue_usd|revenue_cad)",
     "name": "Promo Mix %",     "scale": 1, "format": "percent",
     "formula_tmpl": "SUM({num}) ÷ SUM({den})",
     "description": "Promotional revenue as % of total — tracks promotional dependency."},
    # ── Operations ────────────────────────────────────────────────────────── #
